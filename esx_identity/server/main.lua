...
RegisterServerEvent('sex_identity:setIdentity')
AddEventHandler('sex_identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('sex_identity:identityCheck', myIdentifiers.playerid, true)
		else
...
AddEventHandler('es:playerLoaded', function(source)
	...
	TriggerClientEvent('sex_identity:saveID', source, myID)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('sex_identity:identityCheck', source, false)
			TriggerClientEvent('sex_identity:showRegisterIdentity', source)
		else
			TriggerClientEvent('sex_identity:identityCheck', source, true)
		end
	end)
end)
...
AddEventHandler('onResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		for i=1, #xPlayers, 1 do
			...
			TriggerClientEvent('sex_identity:saveID', xPlayer.source, myID)
			
			getIdentity(xPlayer.source, function(data)
				if data.firstname == '' then
					TriggerClientEvent('sex_identity:identityCheck', xPlayer.source, false)
					TriggerClientEvent('sex_identity:showRegisterIdentity', xPlayer.source)
				else
					TriggerClientEvent('sex_identity:identityCheck', xPlayer.source, true)
				end
			end)
		end
	end
...
TriggerEvent('es:addAdminCommand', 'register', 4, function(source, args, user)
	local player = args[1]
	local xPlayers = ESX.GetPlayers()

	if ESX.GetPlayerFromId(player) then 
		TriggerClientEvent('sex_identity:showRegisterIdentity', player, {})
	else
